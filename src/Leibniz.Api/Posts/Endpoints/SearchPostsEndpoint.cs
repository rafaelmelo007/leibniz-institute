...
public class SearchPostsEndpoint : IEndpoint
{
    ...
    public record SearchPostsResponse(IEnumerable<SearchPostRead> Data, int Index, int Limit, int Count);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var rows = await query.Where(x => postIds.Contains(x.PostId)).OrderBy(x => x.Page).ThenByDescending(x => x.UpdateDateUtc ?? x.CreateDateUtc).Skip(request.Index).Take(request.Limit).ToListAsync();
        ...
        return TypedResults.Ok(new SearchPostsResponse(posts, request.Index, request.Limit, count));
    }
